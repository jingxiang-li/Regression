...
\definecolor{shadecolor}{rgb}{1, 1, 1}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{m_summary} \hlkwb{<-} \hlkwd{summary}\hlstd{(m)}
\hlstd{intcpt_coef} \hlkwb{<-} \hlstd{m_summary}\hlopt{$}\hlstd{coef[}\hlnum{1}\hlstd{,}\hlnum{1}\hlstd{]}
\hlstd{intcpt_sd} \hlkwb{<-} \hlstd{m_summary}\hlopt{$}\hlstd{coef[}\hlnum{1}\hlstd{,}\hlnum{2}\hlstd{]}
\hlstd{t_test} \hlkwb{<-} \hlstd{(intcpt_coef} \hlopt{-} \hlnum{7.9876}\hlstd{)} \hlopt{/} \hlstd{intcpt_sd}
\hlcom{## Using t-test Obtain p-value}
\hlstd{p} \hlkwb{<-} \hlnum{2} \hlopt{*} \hlstd{(}\hlnum{1} \hlopt{-} \hlkwd{pt}\hlstd{(}\hlkwd{abs}\hlstd{(t_test), m}\hlopt{$}\hlstd{df.residual))}
\hlstd{p}
\end{alltt}
\begin{verbatim}
## [1] 0.07373
...
\end{kframe}
\end{knitrout}
Note that the p-value obtained from the t-test is 0.0737, so that it is hard to determine the level of significance of the intercept term. Maybe we need more sample to certify its significance.
...
\hlstd{data}\hlopt{$}\hlstd{Dlic} \hlkwb{<-} \hlnum{1000} \hlopt{*} \hlstd{data}\hlopt{$}\hlstd{Drivers} \hlopt{/} \hlstd{data}\hlopt{$}\hlstd{Pop}
\hlstd{data}\hlopt{$}\hlstd{Fuel} \hlkwb{<-} \hlnum{1000} \hlopt{*} \hlstd{data}\hlopt{$}\hlstd{FuelC} \hlopt{/} \hlstd{data}\hlopt{$}\hlstd{Pop}
\hlstd{data}\hlopt{$}\hlstd{Income} \hlkwb{<-} \hlstd{data}\hlopt{$}\hlstd{Income} \hlopt{/} \hlnum{1000}
\hlstd{data}\hlopt{$}\hlstd{log_miles} \hlkwb{<-} \hlkwd{log}\hlstd{(data}\hlopt{$}\hlstd{Miles,} \hlnum{2}\hlstd{)}
\hlstd{m} \hlkwb{<-} \hlkwd{lm}\hlstd{(Fuel} \hlopt{~} \hlstd{Tax} \hlopt{+} \hlstd{Dlic} \hlopt{+} \hlstd{Income} \hlopt{+} \hlstd{log_miles,} \hlkwc{data} \hlstd{= data)}
